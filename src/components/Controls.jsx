...
function latexToMathJS(latex) {
  ...
  while (expr.includes('\\frac') && maxIterations > 0) {
    ...
    numerator = numerator.replace(/\\placeholder\{\}/g, '').replace(/#\?/g, '').trim();
    denominator = denominator.replace(/\\placeholder\{\}/g, '').replace(/#\?/g, '').trim();

    // Check for empty/incomplete fraction
    if (!numerator || !denominator) {
      throw new Error('Fraction is incomplete - please fill in both numerator and denominator');
    }

    // Replace the fraction with division
    expr = expr.substring(0, fracIndex) +
  ...
  expr = expr.replace(/\\sqrt\[([^\]]+)\]\{([^}]*)\}/g, (match, n, content) => {
    const trimmedContent = content.replace(/\\placeholder\{\}/g, '').replace(/#\?/g, '').trim();
    if (!trimmedContent) {
  ...
  expr = expr.replace(/\\sqrt\{([^}]*)\}/g, (match, content) => {
    const trimmed = content.replace(/\\placeholder\{\}/g, '').replace(/#\?/g, '').trim();
    if (!trimmed) {
      throw new Error('Square root is incomplete - please fill in the content');
    }
    return `sqrt(${trimmed})`;
...
function Controls({
  ...
  useEffect(() => {
    ...
    if (explanationRef.current) {
      const formulas = explanationRef.current.querySelectorAll('.katex-formula');
      formulas.forEach(el => {
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center">
        <h2 className="m-0 mb-2 text-2xl font-semibold" style={{ color: 'var(--text-primary)' }}>
      ...
      <div className="flex flex-col gap-1">
        <label className="flex flex-col gap-1.5 text-sm font-medium" style={{ color: 'var(--text-secondary)' }}>
          <span className="inline whitespace-nowrap">
            Initial Guess (<span ref={x0Ref} className="inline align-baseline text-[0.95em]">x₀</span>):
          </span>
          <input
            type="text"
            value={initialGuess}
      ...
      {isActive && (
        ...
        >
          <div className="flex gap-2 text-sm">
            <span style={{ color: 'var(--text-tertiary)' }}>Current Iteration:</span>
            <span className="font-semibold font-mono" style={{ color: 'var(--success-text)' }}>
              {currentIteration}
            </span>
          </div>
          {hasConverged && (
      ...
      >
        <h3 className="m-0 mb-2 text-sm font-semibold" style={{ color: 'var(--text-secondary)' }}>
          How Newton's Method Works
        </h3>
        <p className="m-0 mb-3 text-[13px] leading-relaxed" style={{ color: 'var(--text-tertiary)' }}>
          Newton's method finds roots (zeros) of a function by repeatedly
          improving an initial guess using tangent lines.
        </p>
        <ol className="m-0 pl-5 text-[13px] leading-loose" style={{ color: 'var(--text-tertiary)' }}>
          <li className="mb-1.5">Start with initial guess <span className="katex-formula" data-latex="x_0"></span></li>
          <li className="mb-1.5">Draw tangent line at <span className="katex-formula" data-latex="(x_n, f(x_n))"></span></li>
          <li className="mb-1.5">Find where tangent crosses x-axis</li>
          <li className="mb-1.5">
            ...
            >
              <span className="katex-formula display-mode" data-latex="x_{n+1} = x_n - \frac{f(x_n)}{f'(x_n)}"></span>
            </div>
          </li>
        </ol>
      </div>
